...
def print_board(board):
    ...
    d={1:"X",-1:"O",0:"."}
    res=board.tolist()
    table=[[d[res[i][j]] for j in range(3)] for i in range(3)]
    for e in table:
        print(*e)
...
def check_row_win(board, player):
    ...
    sms=np.sum(board==player,axis=-1)
    return np.any(sms==3)
...
def check_column_win(board, player):
    ...
    smc=np.sum(board==player,axis=0)
    return np.any(smc==3)
...
def check_anti_diagonal_win(board, player):
    ...
    return board[0,2]==player and board[1,1]==player and board[0,2]==player
